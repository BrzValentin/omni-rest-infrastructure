...
{
    public PublicRestaurantResponse Build(RestaurantEntity restaurant, long version)
    {
...
public sealed class RestaurantStatusCalculator
{
    public PublicRestaurantStatus Calculate(PublicRestaurantResponse restaurant, DateTimeOffset now)
    {
        ...
        return CalculateForCurrentDay(intervals, localDate, localTime, timeZone, "regularHours");
    }
        ...
        string source)
    {
        foreach (var interval in intervals.Where(item => item.ClosesNextDay))
        {
            ...
            {
                return new PublicRestaurantStatus("open", "Open now", ToUtc(currentDate, closes, timeZone), source);
            }
        }
        return null;
    }
        ...
        foreach (var interval in intervals)
        {
            ...
            if (isOpen)
            {
                ...
                return new PublicRestaurantStatus(
                    "open", "Open now", ToUtc(closeDate, closes, timeZone), source);
            }
