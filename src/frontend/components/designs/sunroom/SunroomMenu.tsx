...
export default function SunroomMenu({ site }: WebsiteDesignMenuProps) {
  return (
    <div className={styles.shell} data-website-design="sunroom-v1">
      ...
      <main className={styles.menuMain} id="main-content" lang={site.locale}>
        <header className={styles.menuHero}>
          <p className={styles.eyebrow}>Made for today</p>
        ...
        {!site.menu ? (
          <section className={styles.stateCard} aria-labelledby="sun-no-menu"><h2 id="sun-no-menu">{message("noMenuTitle")}</h2><p>{message("noMenuBody")}</p></section>
        ) : site.menu.categories.length === 0 ? (
          <section className={styles.stateCard} aria-labelledby="sun-no-categories"><h2 id="sun-no-categories">{message("noCategoriesTitle")}</h2><p>{message("noCategoriesBody")}</p></section>
        ) : (
          <>
