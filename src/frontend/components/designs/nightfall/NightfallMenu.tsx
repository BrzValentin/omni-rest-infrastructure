...
import {
  DesignMenuBrowser,
  type DesignMenuClasses,
} from "../shared/DesignMenuBrowser";
import {
  DesignFooter,
  DesignSkipLink,
  HomeLink,
...
export default function NightfallMenu({ site }: WebsiteDesignMenuProps) {
  return (
    <div className={styles.shell} data-website-design="nightfall-v1">
      ...
      <main className={styles.menuMain} id="main-content" lang={site.locale}>
        <header className={styles.menuHero}>
          <p className={styles.eyebrow}>Evening menu</p>
        ...
        />
        {site.taxDisplayMode === "exclusive" ? (
          <p className={styles.notice} role="note">{taxMessage(site.taxNoticeKey)}</p>
        ) : null}
        {!site.menu ? (
          <section className={styles.stateCard} aria-labelledby="night-no-menu">
            <h2 id="night-no-menu">{message("noMenuTitle")}</h2>
            <p>{message("noMenuBody")}</p>
          </section>
        ) : site.menu.categories.length === 0 ? (
          <section className={styles.stateCard} aria-labelledby="night-no-categories">
            <h2 id="night-no-categories">{message("noCategoriesTitle")}</h2>
            <p>{message("noCategoriesBody")}</p>
          </section>
        ) : (
          <>
            <DesignMenuBrowser
              categories={site.menu.categories}
