...
export default function BroadsheetMenu({ site }: WebsiteDesignMenuProps) {
  return (
    <div className={styles.shell} data-website-design="broadsheet-v1">
      ...
      <main className={styles.menuMain} id="main-content" lang={site.locale}>
        <header className={styles.menuHero}>
          <p className={styles.eyebrow}>The daily bill</p>
        ...
        {!site.menu ? (
          <section className={styles.stateCard} aria-labelledby="sheet-no-menu"><h2 id="sheet-no-menu">{message("noMenuTitle")}</h2><p>{message("noMenuBody")}</p></section>
        ) : site.menu.categories.length === 0 ? (
          <section className={styles.stateCard} aria-labelledby="sheet-no-categories"><h2 id="sheet-no-categories">{message("noCategoriesTitle")}</h2><p>{message("noCategoriesBody")}</p></section>
        ) : (
          <>
            <DesignMenuBrowser categories={site.menu.categories} currency={site.currency} locale={site.locale} classes={menuClasses} />
            <p className={styles.disclaimer} role="note">{message("badgesDisclaimer")}</p>
          </>
        )}
      </main>
